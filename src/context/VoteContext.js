...
export const voteReducer = (state = initialState, action) => {
   switch (action.type) {
      ...
      case "VOTE_FOR_CANDIDATE":
         return {
            ...state,
            voter: [...state.voters.filter(c => c.id != action.payload.voter.id), action.payload.voter],
            candidate: [...state.candidates.filter(c => c.id != action.payload.candidate.id), action.payload.candidate],
         }
      case "CLEAN_LISTS":
...
export const VoteContextProvider = ({ children }) => {
   ...
   return (
      <VoteContext.Provider value={{ ...state, addCandidate, addVoter, voteForCandidate, cleanLists }}>
         {children}
